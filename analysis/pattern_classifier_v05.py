...
def compare_stencil_index(
    ...
    """
    Calculate READ displacement relative to WRITE.
...
def is_unit_neighborhood(offset):
    """
    Check whether the offset belongs to a local,
...
def find_stencil_evidence(
    ...
    """
    Detect a regular neighborhood pattern.
    ...
    for write_access in writes:
        ...
        for read_access in reads:
            ...
            #
            # For this first Stencil detector, focus on
            # double-buffer / cross-array stencils.
            #
            # Example:
            #
            # B <- neighborhood of A
            ...
            offset = calculate_stencil_offset(
                write_access,
                read_access
            )
        ...
        for read_array, accesses in (
            ...
            #
            # We need a meaningful neighborhood.
            #
            if (
                not center_present
                or len(neighbors) < 2
            ...
            #
            # Strong stencil evidence:
            #
            # center + multiple local neighbors.
            #
            points = len(
                offsets
            )
